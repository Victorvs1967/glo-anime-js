const categoriesData = () => {
  ...
  const renderAnimeList = (array, ganres) => {
    const product = document.querySelector('.product-page .col-lg-8');
  ...
  fetch('https://anime-app-web-default-rtdb.europe-west1.firebasedatabase.app/anime.json')
    ...
    .then(res => {
      ...
      const param = new URLSearchParams(window.location.search).get('ganre');
      
      res.forEach(anime => ganres.add(anime.ganre));

      renderTopAnime(res.sort((a, b) => a.views < b.views).slice(0, 5));
      param ? renderAnimeList(res, [ param ]) : renderAnimeList(res, ganres);
